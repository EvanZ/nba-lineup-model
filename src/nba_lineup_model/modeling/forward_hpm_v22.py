"""HIPSTER PM v2.2 with empirical usage-allocation context features."""
...
MODEL_NAME = "forward_hpm_v22_usage_allocation"
RUN_PREFIX = "forward-hpm-v22-usage-allocation"
...
def train_hpm_v22(
    *,
    ...
    """Roll HPM v2.2 with empirical rebound and usage allocation features."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "portable-matchup context with depth-aware shooting, empirical "
            "rebound realization, and empirical terminal-usage allocation features"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_V22_USAGE_ALLOCATION,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM v2.2 usage-allocation context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_v22(through_season=args.through_season)
    print(f"HPM v2.2 usage allocation: run={run.run_dir}")
