"""Three-season frozen comparison for NAIL-RAPM v1.3 additive profiles."""
...
from nba_lineup_model.modeling.forward_nail_v13_additive_profiles import MODEL_NAME as CANDIDATE_MODEL_NAME
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v13_additive_profiles_frozen_backtest")


def run_nail_v13_additive_profiles_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay the incumbent and v1.3 with their own forward profile contracts."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen NAIL-RAPM v1.3 additive-profile backtest")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v13_additive_profiles_frozen_backtest(seasons=tuple(args.seasons))
            print(f"NAIL-RAPM v1.3 frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v13_additive_profiles_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.3 frozen backtest: run={run.run_dir}")
