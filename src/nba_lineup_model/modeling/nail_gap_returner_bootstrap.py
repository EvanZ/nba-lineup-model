"""Paired-bootstrap non-promotion gate for NAIL gap-returner priors."""
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_gap_returner_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_gap_returner_bootstrap/2023-24_to_2025-26"
)
...
def run_nail_gap_returner_bootstrap(
    *,
    ...
    """Persist the pooled and seasonal paired-bootstrap non-promotion decision."""
    ...
    gate_passed = bool(gate["non_promotion_gate_passed"].all())
    root = Path(output_root)
    run_id = f"nail-gap-returner-bootstrap-{datetime.now(UTC):%Y%m%dT%H%M%SZ}-{uuid4().hex[:8]}"
    run_dir = root / run_id
    ...
    metadata = {
        ...
        "non_promotion_gate_passed": gate_passed,
...
def _filter_season(source: dict[str, pd.DataFrame], season: str) -> dict[str, pd.DataFrame]:
    return {
        name: frame.loc[frame["season"].eq(season)].copy() for name, frame in source.items()
    }
...
def _latest_directory(root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No frozen gap-returner runs under {root}")
    return candidates[-1]


def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap NAIL gap-returner candidate")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_gap_returner_bootstrap(draws=args.draws, seed=args.seed)
    print(f"NAIL gap-returner bootstrap: run={run_dir}")
