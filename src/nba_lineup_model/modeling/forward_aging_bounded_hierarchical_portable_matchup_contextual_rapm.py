"""Bounded hierarchical portable-matchup contextual RAPM with an aging prior."""
...
MODEL_NAME = "forward_aging_bounded_hierarchical_portable_matchup_contextual_rapm"
RUN_PREFIX = "forward-aging-bounded-hierarchical-portable-matchup-contextual-rapm"


def train_forward_aging_bounded_hierarchical_portable_matchup_contextual_rapm(
    *,
    ...
    """Roll bounded portable context with an age-adjusted returner prior."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_builder=build_aging_exposure_gated_priors,
        player_prior_description=(
            "forward aging-adjusted returning RAPM plus exposure-gated cold-start "
            "prior and bounded portable-matchup context"
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train bounded hierarchical portable-matchup contextual RAPM with "
            "a strictly forward aging prior"
        )
    )
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_forward_aging_bounded_hierarchical_portable_matchup_contextual_rapm(
