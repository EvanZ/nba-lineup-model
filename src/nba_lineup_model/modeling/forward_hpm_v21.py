"""HIPSTER PM v2.1 with empirically calibrated rebound context features."""
...
from nba_lineup_model.modeling.contextual_features import CONTEXT_FEATURE_SET_V21_REBOUND_CAPACITY
...
MODEL_NAME = "forward_hpm_v21_empirical_rebound_capacity"
RUN_PREFIX = "forward-hpm-v21-empirical-rebound-capacity"
...
def train_hpm_v21(
    *,
    ...
    """Roll HPM v2.1 with v2 shooting and empirical rebound realization."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "portable-matchup context with depth-aware shooting and empirically "
            "calibrated offensive and defensive rebound-rate features"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_V21_REBOUND_CAPACITY,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM v2.1 empirical rebound-capacity context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_v21(through_season=args.through_season)
    print(f"HPM v2.1 empirical rebound capacity: run={run.run_dir}")
