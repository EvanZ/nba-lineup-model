"""HPM x2 with one raw offensive-rebounds-per-100 context feature."""
...
from nba_lineup_model.modeling.contextual_features import CONTEXT_FEATURE_SET_X2_ORB_PER_100_TOTAL
...
MODEL_NAME = "forward_hpm_x2_orb_per_100_total"
RUN_PREFIX = "forward-hpm-x2-orb-per-100-total"
...
def train_hpm_x2(
    *,
    ...
    """Roll HPM x2 with only summed player OREB/100 as context."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            "value-conditioned aging RAPM plus exposure-gated cold starts and a "
            "single bounded portable-matchup context feature: summed player OREB/100"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_X2_ORB_PER_100_TOTAL,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM x2 raw OREB/100 context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_x2(through_season=args.through_season)
    print(f"HPM x2 OREB/100 total: run={run.run_dir}")
