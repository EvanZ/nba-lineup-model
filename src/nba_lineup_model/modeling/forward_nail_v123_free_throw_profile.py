"""NAIL-RAPM v1.2.3: v1.2.1 plus free-throw attempts per 100."""
...
MODEL_NAME = "forward_nail_rapm_v123_free_throw_profile"
RUN_PREFIX = "forward-nail-rapm-v123-free-throw-profile"


def train_nail_v123_free_throw_profile(
    *,
    ...
    """Train the strict v1.2.1-plus-FTA/100 additive profile ablation."""
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            f"with {GAP_RETURNER_METHOD}; add free-throw attempts per 100 to the "
            "eight additive player-profile totals"
        ),
        context_fit=fit_linear_ridge_matchup_contextual_model,
        context_metadata=model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_NAIL_V123_FREE_THROW_PROFILE,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train NAIL-RAPM v1.2.3 FTA/100 candidate")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_v123_free_throw_profile(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL-RAPM v1.2.3 free throw: run={run.run_dir}")
            finally:
