"""Paired-bootstrap non-promotion gate for NAIL v1.3 additive profiles."""
...
from nba_lineup_model.modeling.forward_nail_v13_additive_profiles import (
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v13_additive_profiles_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v13_additive_profiles_bootstrap/2023-24_to_2025-26"
)
...
def run_nail_v13_additive_profiles_bootstrap(
    *,
    ...
    run_id = f"nail-v13-additive-profiles-bootstrap-{datetime.now(UTC):%Y%m%dT%H%M%SZ}-{uuid4().hex[:8]}"
    run_dir = root / run_id
...
def _latest_directory(root: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError(f"No frozen v1.3 runs under {root}")
    return candidates[-1]


def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap NAIL v1.3 additive-profile candidate")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v13_additive_profiles_bootstrap(draws=args.draws, seed=args.seed)
    print(f"NAIL v1.3 additive-profile bootstrap: run={run_dir}")
