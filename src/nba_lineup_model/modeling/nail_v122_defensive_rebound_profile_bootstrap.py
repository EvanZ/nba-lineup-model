"""Paired-bootstrap non-promotion gate for NAIL-RAPM v1.2.2."""
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v122_defensive_rebound_profile_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v122_defensive_rebound_profile_bootstrap/2023-24_to_2025-26"
)


def run_nail_v122_defensive_rebound_profile_bootstrap(
    *,
    ...
    """Persist paired bootstrap evidence for the v1.2.2 DRB% ablation."""
    if draws < 1:
    ...
    run_id = (
        "nail-v122-defensive-rebound-profile-bootstrap-"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap NAIL-RAPM v1.2.2 DRB% ablation")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v122_defensive_rebound_profile_bootstrap(
        draws=args.draws,
        seed=args.seed,
    )
    print(f"NAIL-RAPM v1.2.2 bootstrap: run={run_dir}")
