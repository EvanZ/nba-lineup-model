"""Persist and render the continuity-replacement coefficient history."""
...
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/analysis/nail_teammate_continuity_replacement_weight_audit"
)
DEFAULT_CHART_PATH = Path(
    "docs/assets/images/nail-teammate-continuity-replacement/nonadditive-weight-trajectory.svg"
)
FEATURES = ("usage_concentration", "prior_teammate_continuity")


def build_nail_teammate_continuity_replacement_weight_audit(
    *,
    ...
    run = build_nail_critical_spacing_weight_audit(
        ...
        features=FEATURES,
        run_label="nail-teammate-continuity-replacement-weight-audit",
        chart_title="Continuity-replacement context weights by completed source season",
        legend="Orange: retained non-additive or relationship feature",
    )
    candidate = pd.read_parquet(run.run_dir / "seasonal_standardized_weights.parquet")
    incumbent_source = _latest_run(Path(artifacts_dir) / INCUMBENT_MODEL_NAME / "2025-26")
    ...
    matched = (
        incumbent.loc[
            incumbent["feature"].eq("usage_concentration"),
            ["season", "standardized_weight"],
        ]
        .rename(columns={"standardized_weight": "incumbent_weight"})
        .merge(
            candidate.loc[
                candidate["feature"].eq("usage_concentration"),
                ["season", "standardized_weight"],
        ...
        [
            {
                ...
                "removed_incumbent_feature": "top_two_assists",
            }
        ]
    ).to_parquet(run.run_dir / "incumbent_feature_displacement.parquet", index=False)
    return run


def main() -> None:
    parser = argparse.ArgumentParser(description="Audit teammate-continuity replacement weights")
    parser.add_argument("--source-run-dir")
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--chart-path", default=str(DEFAULT_CHART_PATH))
    args = parser.parse_args()
    run = build_nail_teammate_continuity_replacement_weight_audit(
        source_run_dir=args.source_run_dir,
        output_root=args.output_root,
        chart_path=args.chart_path,
    )
    print(
        f"Teammate-continuity replacement weight audit: run={run.run_dir}; chart={run.chart_path}"
    )
