"""HPM x5: retain creation interaction but remove redundant aggregate assists."""
...
MODEL_NAME = "forward_hpm_x5_orb_claim_interaction_creation"
RUN_PREFIX = "forward-hpm-x5-orb-claim-interaction-creation"
...
def train_hpm_x5(
    *,
    ...
    """Roll the interaction-only creation simplification forward."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "ORB-claim rebound replacement, and creation represented only by the "
            "credible-shooter-by-top-two-assists interaction"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_X5_ORB_CLAIM_INTERACTION_CREATION,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM x5 interaction-only creation context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_x5(through_season=args.through_season)
    print(f"HPM x5 interaction-only creation context: run={run.run_dir}")
