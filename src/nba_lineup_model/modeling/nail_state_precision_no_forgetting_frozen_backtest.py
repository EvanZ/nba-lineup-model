"""Three-season no-refit evaluation for State-Precision NAIL without forgetting."""
...
from nba_lineup_model.modeling.forward_nail_state_precision_no_forgetting import (
...
OUTPUT_ARTIFACTS_DIR = Path(
    "artifacts/models/nail_state_precision_no_forgetting_frozen_backtest"
)


def run_nail_state_precision_no_forgetting_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Evaluate stored 2025-26 recursive state for all frozen target seasons.

    The candidate's one completed through-2025-26 run contains every required
    forward prior and context state. This routine only replays those persisted
    states; it never fits any player, context, or schedule model.
    """
    ...
    return run_frozen_multiseason_backtest(
        ...
        models=(
            BacktestModel(
                ...
                "NAIL-RAPM v1.2.1.2 production",
                profile_builder=profiles,
                uses_schedule_control=True,
            ),
            BacktestModel(
                CANDIDATE_MODEL_NAME,
                "State-Precision NAIL (posterior uncertainty, no forgetting)",
                profile_builder=profiles,
                uses_schedule_control=True,
            ),
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay the completed no-forgetting State-Precision NAIL artifact"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_state_precision_no_forgetting_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"State-Precision no-forgetting backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_state_precision_no_forgetting_frozen_backtest(
        seasons=tuple(args.seasons)
    )
    print(f"State-Precision no-forgetting backtest: run={run.run_dir}")
