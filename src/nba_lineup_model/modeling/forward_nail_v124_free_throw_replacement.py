"""NAIL-RAPM v1.2.4: replace usage with free-throw attempts per 100."""
...
MODEL_NAME = "forward_nail_rapm_v124_free_throw_replacement"
RUN_PREFIX = "forward-nail-rapm-v124-free-throw-replacement"


def train_nail_v124_free_throw_replacement(
    *,
    ...
    """Train the v1.2.1 profile with usage replaced by FTA/100."""
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            f"with {GAP_RETURNER_METHOD}; replace usage per 100 with free-throw "
            "attempts per 100 in the eight additive player-profile totals"
        ),
        context_fit=fit_linear_ridge_matchup_contextual_model,
        context_metadata=model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_NAIL_V124_FREE_THROW_REPLACEMENT,
        ...
        profile_contract_metadata={
            **MEDVEDOVSKY_2020_PROFILE_PADDING.metadata(),
            ...
            "additive_contract": {
                "added": ["free_throw_attempts_per_100"],
                "removed": ["usage_per_100"],
                "inherited_from_v121": [
                    "three_pa_per_100",
                    "three_pm_per_100",
                    "assists_per_100",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train NAIL-RAPM v1.2.4 FTA/100 replacement candidate"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_v124_free_throw_replacement(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL-RAPM v1.2.4 free-throw replacement: run={run.run_dir}")
            finally:
