"""Paired-bootstrap non-promotion gate for the NAIL critical-spacing candidate."""
...
from nba_lineup_model.modeling.forward_nail_critical_spacing import (
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_critical_spacing_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_critical_spacing_bootstrap/2023-24_to_2025-26"
)


def run_nail_critical_spacing_bootstrap(
    *,
    ...
    """Persist paired bootstrap evidence for a controlled feature addition."""
    ...
    run_id = f"nail-critical-spacing-bootstrap-{datetime.now(UTC):%Y%m%dT%H%M%SZ}-{uuid4().hex[:8]}"
    run_dir = root / run_id
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap NAIL critical-spacing candidate")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_critical_spacing_bootstrap(draws=args.draws, seed=args.seed)
    print(f"NAIL critical-spacing bootstrap: run={run_dir}")
