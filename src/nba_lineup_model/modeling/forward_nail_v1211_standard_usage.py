"""NAIL-RAPM v1.2.1.1: standard-USG% profile convention."""
...
MODEL_NAME = "forward_nail_rapm_v1211_standard_usage"
RUN_PREFIX = "forward-nail-rapm-v1211-standard-usage"


def train_nail_v1211_standard_usage(
    *,
    ...
    """Train the standard-USG% patch of the pruned NAIL v1.2.1 model."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            "NAIL-RAPM v1.2.1.1 value-conditioned aging and exposure-gated cold "
            f"starts, with {GAP_RETURNER_METHOD}; standard USG% replaces the "
            "legacy usage-events-per-RAPM-possession profile input"
        ),
            ...
            **MEDVEDOVSKY_2020_PROFILE_PADDING.metadata(),
            ...
            "usage_contract": {
                ...
                "pseudo_opportunities": (
                    MEDVEDOVSKY_2020_PROFILE_PADDING.usage_percentage_pseudo_possessions
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train NAIL-RAPM v1.2.1.1 standard-USG% model"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_v1211_standard_usage(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL-RAPM v1.2.1.1 standard USG%: run={run.run_dir}")
            finally:
