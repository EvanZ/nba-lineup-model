"""Bounded hierarchical portable-matchup contextual RAPM with centered aging priors."""
...
from nba_lineup_model.modeling.forward_aging_player_prior import (
    build_centered_aging_exposure_gated_priors,
)
...
MODEL_NAME = "forward_centered_aging_bounded_hierarchical_portable_matchup_contextual_rapm"
RUN_PREFIX = "forward-centered-aging-bounded-hierarchical-portable-matchup-contextual-rapm"


def train_forward_centered_aging_bounded_hierarchical_portable_matchup_contextual_rapm(
    *,
    ...
    """Roll bounded portable context with a centered age-adjusted player prior."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_builder=build_centered_aging_exposure_gated_priors,
        player_prior_description=(
            "prior-season possession-centered aging-adjusted returning RAPM plus "
            "exposure-gated cold-start prior and bounded portable-matchup context"
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train centered aging bounded hierarchical portable-matchup contextual RAPM"
        )
    )
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_forward_centered_aging_bounded_hierarchical_portable_matchup_contextual_rapm(
