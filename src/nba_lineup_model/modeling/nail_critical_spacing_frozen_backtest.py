"""Three-season frozen comparison of NAIL v1.2.1 and critical spacing."""
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_critical_spacing_frozen_backtest")


def run_nail_critical_spacing_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay the critical-spacing candidate against production v1.2.1."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen NAIL critical-spacing comparison")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_critical_spacing_frozen_backtest(seasons=tuple(args.seasons))
            print(f"NAIL critical-spacing frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_critical_spacing_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL critical-spacing frozen backtest: run={run.run_dir}")
