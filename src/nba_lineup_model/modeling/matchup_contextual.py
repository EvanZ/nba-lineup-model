...
class MatchupContextualModel:
    ...
    def predict_side_pairs(self, home: pd.DataFrame, away: pd.DataFrame) -> np.ndarray:
        ...
        relative = _relative_features(home, away)
    ...
    def predict_side_scores(self, side_features: pd.DataFrame) -> np.ndarray:
        ...
        side = _validated_side_features(side_features)
        reference = self.reference_features.loc[:, side_context_feature_columns()]
    ...
    def decompose_side_pairs(self, home: pd.DataFrame, away: pd.DataFrame) -> pd.DataFrame:
        ...
        home = _validated_side_features(home)
        away = _validated_side_features(away)
...
def fit_matchup_contextual_model(
    ...
    *,
    ...
    home = _validated_side_features(home_features)
    away = _validated_side_features(away_features)
    ...
    if curvature_alpha:
        penalty_rows.append(
            np.sqrt(curvature_alpha) * _second_difference_penalty(spline, scale)
        )
    ...
    reference_features, reference_weights = _reference_distribution(home, away, weights)
    return MatchupContextualModel(pipeline, reference_features, reference_weights)
...
class BoundedMatchupContextualModel(MatchupContextualModel):
    ...
    def predict_side_pairs(self, home: pd.DataFrame, away: pd.DataFrame) -> np.ndarray:
        home = _clip_side_features(home, self.side_lower, self.side_upper)
        away = _clip_side_features(away, self.side_lower, self.side_upper)
        relative = _clip_relative_features(_relative_features(home, away), self.relative_cap)
        forward = self.pipeline.predict(relative)
        reverse = self.pipeline.predict(-relative)
...
def fit_bounded_hierarchical_matchup_contextual_model(
    ...
    *,
    ...
    side_lower, side_upper = _side_support_bounds(home, away, weights)
    bounded_home = _clip_side_features(home, side_lower, side_upper)
    bounded_away = _clip_side_features(away, side_lower, side_upper)
    raw_relative = _relative_features(bounded_home, bounded_away)
    relative_cap = _symmetric_relative_caps(raw_relative, weights)
    relative = _clip_relative_features(raw_relative, relative_cap)
    augmented_features = pd.concat([relative, -relative], ignore_index=True)
    augmented_target = np.concatenate([target_values, -target_values])
    augmented_weight = np.concatenate([weights, weights])
    spline = SplineTransformer(n_knots=4, degree=2, extrapolation="linear")
...
def _second_difference_penalty(
    spline: SplineTransformer, scale: StandardScaler
) -> np.ndarray:
...
def _project_previous_response(
    ...
    for feature_index, column in enumerate(contextual_feature_columns()):
        ...
        previous_basis = _feature_basis(previous_spline, feature_index, previous_grid)
        previous_response = (
            (previous_basis - previous_means[start:stop]) / previous_scales[start:stop]
        ) @ previous_coefficients[start:stop]
        current_basis = _feature_basis(spline, feature_index, grid)
...
def _feature_basis(
    spline: SplineTransformer, feature_index: int, values: np.ndarray
) -> np.ndarray:
...
def isolated_feature_component(
    ...
    if isinstance(model, BoundedMatchupContextualModel):
        column = contextual_feature_columns()[feature_index]
        input_values = np.clip(
            input_values,
            -float(model.relative_cap[column]),
            float(model.relative_cap[column]),
        )
    forward_basis = _feature_basis(spline, feature_index, input_values)
    reverse_basis = _feature_basis(spline, feature_index, -input_values)
...
def _relative_features(home: pd.DataFrame, away: pd.DataFrame) -> pd.DataFrame:
    home = _validated_side_features(home)
    away = _validated_side_features(away)
...
def _side_support_bounds(
    home: pd.DataFrame, away: pd.DataFrame, weights: np.ndarray
    ...
    lower = pd.Series(
        [_weighted_quantile(combined[column].to_numpy(float), combined_weights, 0.05) for column in columns],
        index=columns,
    )
    upper = pd.Series(
        [_weighted_quantile(combined[column].to_numpy(float), combined_weights, 0.95) for column in columns],
        index=columns,
    )
    return lower, upper


def _symmetric_relative_caps(relative: pd.DataFrame, weights: np.ndarray) -> pd.Series:
    columns = contextual_feature_columns()
    return pd.Series(
        [
            _weighted_quantile(
...
def _clip_side_features(
    features: pd.DataFrame, lower: pd.Series, upper: pd.Series
) -> pd.DataFrame:
    return _validated_side_features(features).clip(lower, upper, axis=1)


def _clip_relative_features(features: pd.DataFrame, cap: pd.Series) -> pd.DataFrame:
    columns = contextual_feature_columns()
    return features.loc[:, columns].clip(-cap, cap, axis=1)


def _validated_side_features(frame: pd.DataFrame) -> pd.DataFrame:
    columns = side_context_feature_columns()
...
def _reference_distribution(
    ...
    grouped = (
        ...
        .groupby(list(side_context_feature_columns()), as_index=False, sort=False, dropna=False)
