"""Three-season frozen comparison for Kalman NAIL-RAPM v1.4."""
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v14_kalman_additive_profiles_frozen_backtest")


def run_nail_v14_kalman_additive_profiles_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay incumbent v1.2 and Kalman v1.4 under frozen rules."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen Kalman NAIL-RAPM v1.4 backtest")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v14_kalman_additive_profiles_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL-RAPM v1.4 Kalman frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v14_kalman_additive_profiles_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.4 Kalman frozen backtest: run={run.run_dir}")
