"""Paired-bootstrap non-promotion gate for player-trend aging."""
...
from nba_lineup_model.modeling.forward_nail_v1214_player_trend_aging import (
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v1214_player_trend_aging_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v1214_player_trend_aging_bootstrap/2023-24_to_2025-26"
)


def run_nail_v1214_player_trend_aging_bootstrap(
    *,
    ...
    """Persist pooled and per-season paired-bootstrap promotion evidence."""
    ...
    scopes = [("pooled", sources[INCUMBENT_MODEL_NAME], sources[CANDIDATE_MODEL_NAME])]
    scopes.extend(
        (
            season,
        ...
        gate_rows.append(
            {
                ...
                "non_promotion_gate_passed": bool(float(primary["ci_upper"]) <= practical_harm),
            }
        )
    ...
    run_id = (
        "nail-v1214-player-trend-aging-bootstrap-"
        ...
        json.dumps(
            {
                ...
                "relative_harm_threshold": RELATIVE_HARM_THRESHOLD,
                "promotion_rule": (
                    "candidate is eligible when the paired 95% interval upper endpoint "
                    "for candidate-minus-incumbent full-game RMSE is at most +0.5% of "
                    "incumbent RMSE in the pooled and every frozen season"
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap NAIL-RAPM v1.2.1.4 player-trend aging"
    )
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v1214_player_trend_aging_bootstrap(
        draws=args.draws,
        seed=args.seed,
    )
    print(f"NAIL-RAPM v1.2.1.4 player-trend aging bootstrap: run={run_dir}")
