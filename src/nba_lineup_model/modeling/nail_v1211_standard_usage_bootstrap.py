"""Paired-bootstrap non-promotion gate for NAIL-RAPM v1.2.1.1."""
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v1211_standard_usage_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v1211_standard_usage_bootstrap/2023-24_to_2025-26"
)


def run_nail_v1211_standard_usage_bootstrap(
    *,
    ...
    """Persist paired bootstrap evidence for the standard-USG% candidate."""
    if draws < 1:
    ...
    run_id = (
        "nail-v1211-standard-usage-bootstrap-"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap NAIL-RAPM v1.2.1.1 standard-USG% candidate"
    )
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v1211_standard_usage_bootstrap(
        draws=args.draws, seed=args.seed
    )
    print(f"NAIL-RAPM v1.2.1.1 bootstrap: run={run_dir}")
