"""Recursive RAPM with identifiable side-specific lineup-composition context."""
...
from nba_lineup_model.modeling.decomposed_contextual import (
    DecomposedContextualModel,
    fit_decomposed_contextual_model,
    model_metadata,
)
...
MODEL_NAME = "forward_decomposed_contextual_rapm"
RUN_PREFIX = "forward-decomposed-contextual-rapm"


@dataclass(frozen=True)
class ForwardDecomposedContextualRapmRun:
    """One immutable recursive side-context RAPM artifact."""

    run_dir: Path
    run_id: str


def train_forward_decomposed_contextual_rapm(
    *,
...
) -> ForwardDecomposedContextualRapmRun:
    """Roll additive RAPM and side-specific lineup context forward one season at a time."""
    ...
    for season in seasons:
        ...
        if profiles is not None:
            model, row = _fit_decomposed_contextual_season(
                raw_stints, fitted, profiles, context_alpha
            )
    ...
    if target_priors is None or target_profiles is None:
        raise ValueError("Forward decomposed contextual RAPM did not create the target state")
    forecast_model = contextual_models.get(source)
    if forecast_model is None:
        raise ValueError("Forward decomposed contextual RAPM has no source context state")
    ...
    evaluation["source_state"] = {
        **evaluation["source_state"],  # type: ignore[arg-type]
        "player_prior_method": "forward exposure-gated RAPM plus decomposed contextual spline",
        "context_contract": "h_(t-1)(home) minus h_(t-1)(away)",
    }
    return _write_run(
        target=target,
        results=results,
...
def _context_offset(
    stints: pd.DataFrame,
    model: DecomposedContextualModel,
...
def _fit_decomposed_contextual_season(
    stints: pd.DataFrame,
    fitted: ForwardLaggedRapmSeason,
    profiles: pd.DataFrame,
    alpha: float,
) -> tuple[DecomposedContextualModel, dict[str, object]]:
...
def _write_run(
    *,
    ...
    root = artifacts_dir / "forward_decomposed_contextual_rapm" / target
    output = root / run_id
    ...
    try:
        ...
        metadata = {
            ...
            "context_alpha": context_alpha,
            "contextual_offset_contract": "h_(t-1)(home) minus h_(t-1)(away)",
            "created_at": now.isoformat(),
            "code_version": modeling_code_fingerprint(
                (Path(__file__), Path(__file__).with_name("decomposed_contextual.py"))
            ),
        ...
        return ForwardDecomposedContextualRapmRun(output, run_id)
    except Exception:
        if temporary.exists():
...
def main() -> None:
    """Train the published decomposed-context RAPM exemplar."""

    parser = argparse.ArgumentParser(
        description="Train forward RAPM with decomposed side-specific lineup context"
    )
    ...
    run = train_forward_decomposed_contextual_rapm(
        through_season=args.through_season,
        context_alpha=args.context_alpha,
    )
    print(f"Forward decomposed contextual RAPM: run={run.run_dir}")
