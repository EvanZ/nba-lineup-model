"""Three-season frozen ablation of NAIL-RAPM v1.3.1 versus v1.3."""
...
from nba_lineup_model.modeling.forward_nail_v131_pruned_additive_profiles import (
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v131_pruned_additive_profiles_frozen_backtest")


def run_nail_v131_pruned_additive_profiles_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay v1.3 and its two-feature additive-profile ablation under frozen rules."""
    ...
    return run_frozen_multiseason_backtest(
        ...
        models=(
            ...
            BacktestModel(
                ...
                "NAIL-RAPM v1.3.1 pruned additive profiles",
                profile_builder=profiles,
            ),
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen NAIL-RAPM v1.3.1 pruned-profile backtest"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v131_pruned_additive_profiles_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL-RAPM v1.3.1 frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v131_pruned_additive_profiles_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.3.1 frozen backtest: run={run.run_dir}")
