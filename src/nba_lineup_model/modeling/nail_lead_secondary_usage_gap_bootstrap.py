"""Paired-bootstrap gate for the lead-secondary usage-gap NAIL candidate."""
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_lead_secondary_usage_gap_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_lead_secondary_usage_gap_bootstrap/2023-24_to_2025-26"
)


def run_nail_lead_secondary_usage_gap_bootstrap(
    *,
    ...
    run_id = (
        "nail-lead-secondary-usage-gap-bootstrap-"
        ...
        json.dumps(
            {
                ...
                "relative_harm_threshold": RELATIVE_HARM_THRESHOLD,
                "promotion_rule": (
                    "The candidate is eligible only when the upper endpoint of the paired 95% "
                    "confidence interval for candidate-minus-incumbent full-game RMSE is at most "
                    "+0.5% of incumbent RMSE in the pooled and each frozen season."
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap lead-secondary usage-gap candidate")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_lead_secondary_usage_gap_bootstrap(draws=args.draws, seed=args.seed)
    print(f"NAIL lead-secondary usage-gap bootstrap: run={run_dir}")
