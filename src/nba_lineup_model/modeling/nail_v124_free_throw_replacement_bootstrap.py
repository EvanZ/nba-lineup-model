"""Paired-bootstrap non-promotion gate for NAIL-RAPM v1.2.4."""
...
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v124_free_throw_replacement_bootstrap/2023-24_to_2025-26"
)


def run_nail_v124_free_throw_replacement_bootstrap(
    *,
    ...
    """Persist paired bootstrap evidence for the v1.2.4 replacement experiment."""
    if draws < 1:
    ...
    run_id = (
        "nail-v124-free-throw-replacement-bootstrap-"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap NAIL-RAPM v1.2.4 FTA/100 replacement"
    )
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v124_free_throw_replacement_bootstrap(
        draws=args.draws,
        seed=args.seed,
    )
    print(f"NAIL-RAPM v1.2.4 bootstrap: run={run_dir}")
