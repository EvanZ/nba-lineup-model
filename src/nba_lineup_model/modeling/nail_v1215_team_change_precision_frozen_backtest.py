"""Three-season frozen comparison for team-change prior precision."""
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v1215_team_change_precision_frozen_backtest")


def run_nail_v1215_team_change_precision_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Compare clean-offseason team-change precision with current production."""
    ...
    return run_frozen_multiseason_backtest(
        ...
        models=(
            ...
            BacktestModel(
                ...
                "NAIL-RAPM v1.2.1.5 team-change precision",
                profile_builder=profiles,
                uses_schedule_control=True,
            ),
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen NAIL v1.2.1.5 team-change precision comparison"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v1215_team_change_precision_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL-RAPM v1.2.1.5 frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v1215_team_change_precision_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.2.1.5 frozen backtest: run={run.run_dir}")
