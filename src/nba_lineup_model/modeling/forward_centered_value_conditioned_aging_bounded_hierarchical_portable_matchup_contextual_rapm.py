"""Bounded portable contextual RAPM with value-conditioned aging priors."""
...
def train_value_conditioned_aging_hpm(
    *,
    ...
    """Roll centered portable context with a value-conditioned aging prior."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train centered value-conditioned aging bounded portable contextual RAPM"
        )
    )
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_value_conditioned_aging_hpm(through_season=args.through_season)
    print(f"Forward centered value-conditioned aging portable RAPM: run={run.run_dir}")
