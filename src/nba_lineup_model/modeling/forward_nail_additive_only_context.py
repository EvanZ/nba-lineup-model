"""NAIL-RAPM ablation retaining only lineup-additive basketball profile terms."""
...
from nba_lineup_model.modeling.contextual_features import CONTEXT_FEATURE_SET_NAIL_ADDITIVE_ONLY
...
MODEL_NAME = "forward_nail_additive_only_context"
RUN_PREFIX = "forward-nail-additive-only-context"


def train_nail_additive_only_context(
    *,
    ...
    """Train NAIL with the six non-additive lineup coordinates removed."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            "value-conditioned aging RAPM plus exposure-gated cold starts; only the "
            "eight basketball-additive NAIL profile coordinates remain in lineup context"
        ),
        context_fit=fit_linear_ridge_matchup_contextual_model,
        context_metadata=model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_NAIL_ADDITIVE_ONLY,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train NAIL-RAPM additive-only context ablation")
    ...
    run = train_nail_additive_only_context(
        through_season=args.through_season,
        context_alpha=args.context_alpha,
    )
    print(f"NAIL-RAPM additive-only context: run={run.run_dir}")
