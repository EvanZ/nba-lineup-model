"""HIPSTER PM v2 with depth-aware shooting context features."""
...
MODEL_NAME = "forward_hpm_v2_depth_aware_shooting"
RUN_PREFIX = "forward-hpm-v2-depth-aware-shooting"
...
def train_hpm_v2(
    *,
    ...
    """Roll HPM v2 with the incumbent player prior and depth-aware context."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "portable-matchup context with depth-aware shooting features"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_V2_DEPTH_AWARE_SHOOTING,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM v2 depth-aware shooting context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_v2(through_season=args.through_season)
    print(f"HPM v2 depth-aware shooting: run={run.run_dir}")
