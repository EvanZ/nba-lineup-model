"""Three-season frozen comparison of NAIL-RAPM v1.2.2 against v1.2.1."""
...
from nba_lineup_model.modeling.forward_nail_v122_defensive_rebound_profile import (
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v122_defensive_rebound_profile_frozen_backtest")


def run_nail_v122_defensive_rebound_profile_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay the strict v1.2.1-plus-DRB% candidate under frozen rules."""
    profiles = partial(
        build_contextual_player_profiles,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen NAIL-RAPM v1.2.2 DRB% ablation")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v122_defensive_rebound_profile_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL-RAPM v1.2.2 frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v122_defensive_rebound_profile_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.2.2 frozen backtest: run={run.run_dir}")
