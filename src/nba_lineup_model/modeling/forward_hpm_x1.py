"""HPM x1 with one ORB-claim context feature."""
...
from nba_lineup_model.modeling.contextual_features import CONTEXT_FEATURE_SET_X1_ORB_CLAIM_TOTAL
...
MODEL_NAME = "forward_hpm_x1_orb_claim_total"
RUN_PREFIX = "forward-hpm-x1-orb-claim-total"
...
def train_hpm_x1(
    *,
    ...
    """Roll HPM x1 with only summed individual ORB% claims as context."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "single bounded portable-matchup context feature: summed player ORB% claims"
        ),
        context_fit=fit_bounded_hierarchical_matchup_contextual_model,
        context_metadata=bounded_model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_X1_ORB_CLAIM_TOTAL,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train HPM x1 ORB-claim context")
    parser.add_argument("--through-season", default=DEFAULT_TARGET_SEASON)
    args = parser.parse_args()
    run = train_hpm_x1(through_season=args.through_season)
    print(f"HPM x1 ORB claim total: run={run.run_dir}")
