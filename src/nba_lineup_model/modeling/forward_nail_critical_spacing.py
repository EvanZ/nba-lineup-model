"""Controlled NAIL-RAPM candidate: add a critical-spacing lineup indicator."""
...
MODEL_NAME = "forward_nail_rapm_critical_spacing"
RUN_PREFIX = "forward-nail-critical-spacing"


def train_nail_critical_spacing(
    *,
    ...
    """Fit the v1.2.1 critical-spacing candidate over the recursive history."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            f"with {GAP_RETURNER_METHOD}; add a critical-spacing indicator to "
            "the retained usage-concentration and top-two-assists context terms"
        ),
        context_fit=fit_linear_ridge_matchup_contextual_model,
        context_metadata=model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_NAIL_CRITICAL_SPACING,
            ...
            **MEDVEDOVSKY_2020_PROFILE_PADDING.metadata(),
            ...
            "nonadditive_contract": {
                ...
                "critical_spacing_definition": (
                    ...
                    "strictly below the prior-season profile pool's lower tercile"
                ),
                "shooting_feature": "three_pm_per_100",
                "low_threat_quantile": 1.0 / 3.0,
                "activation_count": 2,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train NAIL critical-spacing candidate")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_critical_spacing(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL critical-spacing candidate: run={run.run_dir}")
            finally:
