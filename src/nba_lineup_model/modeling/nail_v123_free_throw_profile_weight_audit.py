"""Audit NAIL-RAPM v1.2.3 additive and non-additive coefficient trajectories."""
...
from nba_lineup_model.modeling.forward_nail_v123_free_throw_profile import MODEL_NAME
...
DEFAULT_ADDITIVE_CHART_PATH = Path(
    "docs/assets/images/nail-v123/additive-profile-weight-trajectories.svg"
)
DEFAULT_NONADDITIVE_CHART_PATH = Path(
    "docs/assets/images/nail-v123/retained-nonadditive-weight-trajectories.svg"
)
...
class NailV123FreeThrowProfileWeightAuditRun:
    """Persisted v1.2.3 standardized coefficient audit."""
...
def build_nail_v123_free_throw_profile_weight_audit(
    *,
...
) -> NailV123FreeThrowProfileWeightAuditRun:
    """Extract each v1.2.3 source-season Ridge coefficient and render both panels."""
    source = (
        Path(source_run_dir)
    ...
    if metadata.get("model") != MODEL_NAME:
        raise ValueError("Weight audit requires a NAIL-RAPM v1.2.3 artifact")
    models = joblib.load(source / "season_context_models.joblib")
    additive_features = tuple(LINEAR_NAIL_V123_BASKETBALL_ADDITIVE_FEATURES)
    ...
    render_additive_weight_trajectories(
        ...
        title="NAIL-RAPM v1.2.3 additive profile weights by completed source season",
        features=additive_features,
        accent_features=frozenset(),
    ...
    render_additive_weight_trajectories(
        ...
        title="NAIL-RAPM v1.2.3 retained non-additive weights by completed source season",
        features=RETAINED_NONADDITIVE_FEATURES,
        accent_features=frozenset(RETAINED_NONADDITIVE_FEATURES),
    ...
    run_id = (
        "nail-v123-free-throw-profile-weight-audit-"
    ...
    return NailV123FreeThrowProfileWeightAuditRun(
        run_dir,
        source,
        rendered_additive_chart,
        rendered_nonadditive_chart,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit NAIL-RAPM v1.2.3 coefficient weights")
    ...
    run = build_nail_v123_free_throw_profile_weight_audit(
    ...
    print(
        "NAIL-RAPM v1.2.3 coefficient audit: "
