"""Three-season frozen comparison for the player-trend aging candidate."""
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v1214_player_trend_aging_frozen_backtest")


def run_nail_v1214_player_trend_aging_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Compare the trend-aging candidate with the current production model."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen NAIL v1.2.1.4 player-trend aging comparison"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v1214_player_trend_aging_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(
                f"NAIL-RAPM v1.2.1.4 player-trend aging backtest: run={run.run_dir}",
                flush=True,
            )
        return
    run = run_nail_v1214_player_trend_aging_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL-RAPM v1.2.1.4 player-trend aging backtest: run={run.run_dir}")
