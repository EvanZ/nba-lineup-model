"""Linear-Ridge contextual RAPM using the HPM x3 feature contract."""
...
MODEL_NAME = "forward_hpm_x3_linear_ridge_context"
RUN_PREFIX = "forward-hpm-x3-linear-ridge-context"


def train_hpm_x3_linear_ridge(
    *,
    ...
    """Roll the canonical 14-feature HPM x3 contract through linear Ridge."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            "canonical 14-feature HPM x3 basketball context fit as standardized linear Ridge"
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train linear-Ridge HPM x3 context")
    ...
    run = train_hpm_x3_linear_ridge(
        through_season=args.through_season,
        context_alpha=args.context_alpha,
    )
    print(f"Linear-Ridge HPM x3 context: run={run.run_dir}")
