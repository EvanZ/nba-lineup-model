"""Paired-bootstrap non-promotion gate for NAIL-RAPM v1.3.2."""
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v132_dynamic_additive_profiles_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v132_dynamic_additive_profiles_bootstrap/2023-24_to_2025-26"
)


def run_nail_v132_dynamic_additive_profiles_bootstrap(
    *,
    ...
    """Persist paired-bootstrap gate results against the v1.3.1 parent."""
    if draws < 1:
    ...
    run_id = (
        "nail-v132-dynamic-additive-profiles-bootstrap-"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bootstrap NAIL-RAPM v1.3.2 dynamic profiles")
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v132_dynamic_additive_profiles_bootstrap(
        draws=args.draws, seed=args.seed
    )
    print(f"NAIL-RAPM v1.3.2 bootstrap: run={run_dir}")
