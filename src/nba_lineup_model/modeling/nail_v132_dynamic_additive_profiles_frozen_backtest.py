"""Three-season frozen comparison of NAIL-RAPM v1.3.2 versus v1.3.1."""
...
from nba_lineup_model.modeling.forward_nail_v132_dynamic_additive_profiles import (
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_v132_dynamic_additive_profiles_frozen_backtest")


def run_nail_v132_dynamic_additive_profiles_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay v1.3.1 and its dynamic-state descendant under frozen rules."""
    profiles = partial(
        build_contextual_player_profiles,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run frozen NAIL-RAPM v1.3.2 dynamic-profile backtest"
    )
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_v132_dynamic_additive_profiles_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL-RAPM v1.3.2 frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_v132_dynamic_additive_profiles_frozen_backtest(
        seasons=tuple(args.seasons)
    )
    print(f"NAIL-RAPM v1.3.2 frozen backtest: run={run.run_dir}")
