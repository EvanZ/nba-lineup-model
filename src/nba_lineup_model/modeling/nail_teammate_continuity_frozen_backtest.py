"""Three-season frozen comparison for prior teammate continuity."""
...
OUTPUT_ARTIFACTS_DIR = Path("artifacts/models/nail_teammate_continuity_frozen_backtest")


def run_nail_teammate_continuity_frozen_backtest(
    *,
    seasons: tuple[str, ...] = DEFAULT_SEASONS,
    artifacts_dir: Path | str = DEFAULT_ARTIFACTS_DIR,
    output_artifacts_dir: Path | str = OUTPUT_ARTIFACTS_DIR,
) -> FrozenMultiseasonBacktestRun:
    """Replay the one-feature candidate against production on shared support."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen teammate-continuity comparison")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle, redirect_stdout(handle):
            run = run_nail_teammate_continuity_frozen_backtest(
                seasons=tuple(args.seasons)
            )
            print(f"NAIL teammate-continuity frozen backtest: run={run.run_dir}", flush=True)
        return
    run = run_nail_teammate_continuity_frozen_backtest(seasons=tuple(args.seasons))
    print(f"NAIL teammate-continuity frozen backtest: run={run.run_dir}")
