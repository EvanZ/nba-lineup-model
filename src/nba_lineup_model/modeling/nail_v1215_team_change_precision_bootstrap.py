"""Paired-bootstrap non-promotion gate for team-change prior precision."""
...
from nba_lineup_model.modeling.forward_nail_v1215_team_change_precision import (
...
DEFAULT_BACKTEST_ROOT = Path(
    "artifacts/models/nail_v1215_team_change_precision_frozen_backtest/"
    "frozen_multiseason_backtest/2023-24_to_2025-26"
)
DEFAULT_OUTPUT_ROOT = Path(
    "artifacts/models/nail_v1215_team_change_precision_bootstrap/2023-24_to_2025-26"
)


def run_nail_v1215_team_change_precision_bootstrap(
    *,
    ...
    run_id = (
        "nail-v1215-team-change-precision-bootstrap-"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bootstrap NAIL-RAPM v1.2.1.5 team-change precision"
    )
    parser.add_argument("--draws", type=int, default=DEFAULT_DRAWS)
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED)
    args = parser.parse_args()
    run_dir = run_nail_v1215_team_change_precision_bootstrap(
        draws=args.draws,
        seed=args.seed,
    )
    print(f"NAIL-RAPM v1.2.1.5 team-change precision bootstrap: run={run_dir}")
