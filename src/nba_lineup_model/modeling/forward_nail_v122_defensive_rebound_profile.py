"""NAIL-RAPM v1.2.2: v1.2.1 plus defensive-rebound percentage."""
...
MODEL_NAME = "forward_nail_rapm_v122_defensive_rebound_profile"
RUN_PREFIX = "forward-nail-rapm-v122-defensive-rebound-profile"


def train_nail_v122_defensive_rebound_profile(
    *,
    ...
    """Train the strict v1.2.1-plus-DRB% additive profile ablation."""
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            f"with {GAP_RETURNER_METHOD}; add defensive-rebound percentage to the "
            "eight additive player-profile totals"
        ),
        context_fit=fit_linear_ridge_matchup_contextual_model,
        context_metadata=model_metadata,
        context_feature_set=CONTEXT_FEATURE_SET_NAIL_V122_DEFENSIVE_REBOUND_PROFILE,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train NAIL-RAPM v1.2.2 DRB% candidate")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_v122_defensive_rebound_profile(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL-RAPM v1.2.2 defensive rebound: run={run.run_dir}")
            finally:
