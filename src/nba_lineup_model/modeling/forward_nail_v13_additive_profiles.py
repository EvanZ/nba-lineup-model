"""NAIL-RAPM v1.3 candidate with four additional additive player profiles."""
...
MODEL_NAME = "forward_nail_rapm_v13_additive_profiles"
RUN_PREFIX = "forward-nail-rapm-v13-additive-profiles"
...
def train_nail_v13_additive_profiles(
    *,
    ...
    """Fit the controlled v1.3 expanded-additive-profile candidate."""
    ...
    return train_forward_portable_matchup_contextual_rapm(
        ...
        player_prior_description=(
            ...
            f"with {GAP_RETURNER_METHOD}; four additional lagged additive profiles: "
            "DRB%, FTA/100, unassisted rim makes/100, and unassisted three makes/100"
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train NAIL-RAPM v1.3 additive-profile candidate")
    ...
    if args.log_path:
        ...
        with log_path.open("w") as handle:
            ...
            try:
                run = train_nail_v13_additive_profiles(
                    through_season=args.through_season,
                    context_alpha=args.context_alpha,
                )
                print(f"NAIL-RAPM v1.3 additive-profile candidate: run={run.run_dir}")
            finally:
