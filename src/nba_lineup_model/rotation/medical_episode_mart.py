"""Construct medical-absence episode and player-season actuarial targets."""
...
DEFAULT_OUTPUT_DIR = Path("data/analytical/medical_availability_episodes")
DEFAULT_SEASONS = tuple(f"{year}-{str(year + 1)[-2:]}" for year in range(2015, 2026))


def build_medical_episode_frame(player_games: pd.DataFrame) -> pd.DataFrame:
    """Return injury/illness episodes from one or more player-game seasons.

    A one-game coach-DNP state directly bracketed by injury/illness rows is
    reconciled into the same medical episode. The source mart applies this
    ...
    corrected = _prepare_medical_episode_rows(player_games)
    if corrected.empty:
        return _empty_episode_frame()

    medical = corrected.loc[corrected["is_medical_absence"]].copy()
    if medical.empty:
        return _empty_episode_frame()
    episodes = (
        medical.groupby(
            ["player_id", "medical_episode_number"],
            as_index=False,
            sort=False,
        )
        .agg(
            ...
            medical_missed_games=("game_id", "size"),
            reconciled_coach_dnp_games=("is_reconciled_coach_dnp", "sum"),
            final_medical_game_order=("game_order", "max"),
            final_known_game_order=("last_known_game_order", "max"),
        )
        .sort_values(
            ["episode_start_season", "player_id", "medical_episode_number"], kind="stable"
        )
        .reset_index(drop=True)
    )
    episodes["medical_episode_number"] = episodes["medical_episode_number"].astype(int)
    episodes["medical_missed_games"] = episodes["medical_missed_games"].astype(int)
    episodes["reconciled_coach_dnp_games"] = episodes["reconciled_coach_dnp_games"].astype(int)
    episodes["right_censored"] = episodes["final_medical_game_order"].eq(
        episodes["final_known_game_order"]
    )
    episodes["episode_id"] = episodes.apply(
        lambda row: (
            f"{row.episode_start_season}:{int(row.player_id)}:"
            f"{int(row.medical_episode_number)}"
        ),
        axis=1,
    )
    return episodes.loc[
...
def build_medical_episode_season_summary(
    ...
    *,
    ...
    The episode count belongs to the season in which an episode began. Medical
    missed games remain allocated to the actual season/game in which they occur.
    """

    known = _prepare_medical_episode_rows(player_games)
    if known.empty:
        return pd.DataFrame()
    ...
    medical = known.loc[known["is_medical_absence"]].copy()
    episode_summary = (
        medical.groupby(["season", "player_id"], as_index=False, sort=False)
        .agg(
            medical_episode_count=("is_medical_episode_start", "sum"),
            medical_missed_games=("game_id", "size"),
            reconciled_coach_dnp_games=("is_reconciled_coach_dnp", "sum"),
        )
    ...
    episode_starts = (
        ...
        .agg(
            mean_episode_missed_games=("medical_missed_games", "mean"),
            max_episode_missed_games=("medical_missed_games", "max"),
            censored_medical_episode_count=("right_censored", "sum"),
        )
    ...
    if episode_summary.empty:
        episode_summary = pd.DataFrame(
            columns=[
                "season",
                "player_id",
                "medical_episode_count",
                "medical_missed_games",
                "reconciled_coach_dnp_games",
                "mean_episode_missed_games",
                "max_episode_missed_games",
                "censored_medical_episode_count",
            ]
        )
    else:
        episode_summary["medical_episode_count"] = episode_summary[
            "medical_episode_count"
        ].fillna(0)
        episode_summary["medical_missed_games"] = episode_summary[
            "medical_missed_games"
        ].fillna(0)
    ...
    summary["medical_loss_cost"] = (
        summary["medical_missed_games"] / summary["rostered_team_games"]
    )
    ...
    if player_panel is not None:
        ...
        panel = player_panel.loc[:, sorted(required_panel)].copy()
...
def build_medical_episode_mart(
    *,
    ...
    episodes = build_medical_episode_frame(player_games)
    panel_path = Path(player_panel_path)
    panel = pd.read_parquet(panel_path) if panel_path.exists() else None
    return episodes, build_medical_episode_season_summary(
        player_games,
        episodes,
        player_panel=panel,
    )
...
def _prepare_medical_episode_rows(player_games: pd.DataFrame) -> pd.DataFrame:
    ...
    corrected["is_medical_absence"] = corrected["availability_state"].eq(
        STATE_UNAVAILABLE_INJURY
    )
    groups = corrected.groupby("player_id", sort=False)
    previous_medical = groups["is_medical_absence"].shift(1, fill_value=False).astype(bool)
    corrected["is_medical_episode_start"] = (
        corrected["is_medical_absence"] & ~previous_medical
    )
    corrected["medical_episode_number"] = corrected["is_medical_episode_start"].astype(
        int
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build medical availability episode mart")
    ...
    episodes, summary = build_medical_episode_mart(
        seasons=seasons,
    ...
    print(f"Wrote {len(episodes):,} medical episodes and {len(summary):,} player seasons")
    return 0
