...
from nba_lineup_model.rotation.medical_episode_mart import (
    build_medical_episode_frame,
    build_medical_episode_season_summary,
)
    ...
    return pd.DataFrame(
        {
            ...
            "available": [state != STATE_UNAVAILABLE_INJURY for state in states],
...
def test_bridges_isolated_coach_dnp_inside_medical_episode() -> None:
    games = _player_games(
        [STATE_PLAYED, STATE_UNAVAILABLE_INJURY, STATE_AVAILABLE_DNP_COACH, STATE_UNAVAILABLE_INJURY, STATE_PLAYED]
    )

    episodes = build_medical_episode_frame(games)

    assert len(episodes) == 1
    episode = episodes.iloc[0]
    assert episode["medical_missed_games"] == 3
...
def test_distinct_medical_absences_remain_separate_episodes() -> None:
    ...
    assert episodes["medical_missed_games"].tolist() == [1, 1]
    assert episodes["right_censored"].tolist() == [False, True]


def test_medical_episode_continues_across_seasons() -> None:
    ...
    episodes = build_medical_episode_frame(games)
    summary = build_medical_episode_season_summary(games, episodes)

    assert len(episodes) == 1
    episode = episodes.iloc[0]
    assert episode["episode_start_season"] == "2024-25"
    assert episode["episode_end_season"] == "2025-26"
    assert episode["medical_missed_games"] == 2
    values = summary.set_index("season")
    assert values.loc["2024-25", "medical_episode_count"] == 1
    assert values.loc["2025-26", "medical_episode_count"] == 0
    assert values.loc["2025-26", "medical_missed_games"] == 1
...
def test_player_season_summary_uses_episode_counts_and_panel_gp_gs() -> None:
    ...
    summary = build_medical_episode_season_summary(games, episodes, player_panel=panel).iloc[0]

    assert summary["medical_episode_count"] == 1
    assert summary["medical_missed_games"] == 3
